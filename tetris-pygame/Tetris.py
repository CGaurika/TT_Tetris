...
def get_max_score():
    try:
        ...
        pass  # Handle the case where the file doesn't exist or is empty
    return 0  # Default score if file is empty or doesn't exist
...
def main(window):
    ...
    fall_time = 0
    fall_speeds = {"Easy": 0.5, "Medium": 0.35, "Hard": 0.1}  # Define fall speeds for each difficulty level
    fall_speed = fall_speeds[difficulty]  # Set initial fall speed based on selected difficulty level
    level_time = 0
    score = 0
    ...
    while run:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # New: Pause game on 'p' key
                elif event.key == pygame.K_p:
                    paused = not paused

                # New: Replay on 'r' key
                elif event.key == pygame.K_r:
                    return  # Exit current game and return to main menu

        if paused:
            draw_text_middle('Paused. Press "p" to resume.', 40, (255, 255, 255), window)
        else:
    ...
    draw_text_middle(f'You Lost. Difficulty: {difficulty}', 40, (255, 255, 255), window)
    pygame.display.update()
...
def main_menu(window):
    ...
    font = pygame.font.Font(fontpath, 50)  # Initialize the font
    ...
    while run:
        ...
        for i, option in enumerate(options):
            draw_text = font.render(option, 1, (255, 255, 255))
            window.blit(draw_text, (top_left_x, start_y + i * line_spacing))
